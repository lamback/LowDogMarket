...
function onOrderReady(){
    $.ajax({
        ...
        error: function(){
            resultJSON={data:[
                    {goodsid:12,name:'男士青年外套中长款上衣', type:'女装男装', photo:'image/goods/goods03.jpg', information:'加绒更保暖',price:'122.98', seller:'商家1', sellnumber:'12', address:'赣州',number:'2',buydate:'2018-12-01',iscomment:'1'},
            ...
            var orderContainer=document.getElementById("orderContainer");
            for(var i=0;resultJSON.data.length;i++){
                orderContainer.innerHTML+='<div class="order-list">\n' +
                    '        <div class="order-list-header">\n' +
                    '            <p class="time-id"><samp>&nbsp;&nbsp;下单时间：</samp><samp class="time">'+dresultJSON.data[i].buydate+'</samp><samp class="id">&nbsp;&nbsp;订单号：</samp><samp class="idmunber">'+getOrderID()+'</samp></p>\n' +
                    ...
                    '            <p class="list6"><samp class="infosamp" >订单已完成！</samp></p>\n' +
                    '            <p class="list7">'+isComment(resultJSON.data[i].iscomment,resultJSON.data[i].goodsid)+'</button></p>\n' +
                    '        </div>\n' +
                    '    </div>\n' +
                    '    <div class="list-in-clear"></div>'
