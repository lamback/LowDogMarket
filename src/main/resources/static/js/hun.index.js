...
window.onload = function () {
    ...
    for (var i = 0; i < lilist.length; i++) {          //生成排行榜
        ...
        lilist[i].innerHTML+="<div class='pic f1'><a href='#'>";
        $.ajax({
            type: "POST",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "/salesRanking" ,//url
        ...
        lilist[i].innerHTML+="<img src="+photo+"/></a></div>"+"<div class='title w-all fl m-t-18'><a href='//localhost:8080/good.html?goodsid="+goodsid+"' class='dsblock w-all fl'> ";
        lilist[i].innerHTML+="<h3 class='fs-14 ftc-787878 text-l line-h18 tw_hidden fl'>"+information+"</h3></a></div>";
